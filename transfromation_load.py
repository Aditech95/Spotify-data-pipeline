...
def load_to_postgres(album_df, artists_df, song_df):
    ...
    for _, row in album_df.iterrows():
        cur.execute("""
            INSERT INTO albums (album_id, album_name, release_date, total_tracks)
            VALUES (%s, %s, %s, %s)
            ON CONFLICT (album_id) DO NOTHING
        """, (row['album_id'], row['album_name'], row['release_date'], row['total_tracks']))
    ...
    for _, row in song_df.iterrows():
        cur.execute("""
            INSERT INTO songs (song_id, song_name, duration_ms, added_at, album_id, artist_id)
            VALUES (%s, %s, %s, %s, %s, %s)
            ON CONFLICT (song_id) DO NOTHING
        """, (row['song_id'], row['song_name'], row['duration_ms'], row['added_at'], row['album_id'], row['artist_id']))
